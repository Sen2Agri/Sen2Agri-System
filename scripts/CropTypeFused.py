...
#     executeStep("ogr2ogr", "/usr/local/bin/ogr2ogr", "-clipsrc", shape, "-progress", "-overwrite",reference_polygons_clip, reference_polygons_reproject, skip=fromstep>7)
    ...
    executeStep("ogr2ogr", "ogr2ogr", reference_polygons_clip, reference_polygons)
...
#Train Image Classifier (Step 10)
    if classifier == "rf" :
        executeStep("TrainImagesClassifier", "otbcli", "CropTypeTrainImagesClassifier", buildFolder,
                    "-nodatalabel", "-10000", "-outstat", statistics, "-pixsize", pixsize, "-mode",
                    trm,
                    "-io.vd",training_polygons,"-io.imstat", statistics, "-rand", random_seed,
                    "-sample.bm", "0", "-io.confmatout", confmatout,"-io.out",model,"-sample.mt",
                    "-1","-sample.mv","-1","-sample.vfn","CODE","-sample.vtr","0.1","-classifier","rf",
                    "-classifier.rf.nbtrees",rfnbtrees,"-classifier.rf.min",
                    rfmin,"-classifier.rf.max",rfmax, *(["-sp"] + sp + ["-il"] + indesc), skip=fromstep>10)
    elif classifier == "svm":
        executeStep("TrainImagesClassifier", "otbcli", "CropTypeTrainImagesClassifier", buildFolder,
                    "-nodatalabel", "-10000", "-outstat", statistics, "-pixsize", pixsize, "-mode",
                    trm,
                    "-io.vd",training_polygons,"-io.imstat", statistics, "-rand",
                    "-sample.bm", "0", "-io.confmatout", confmatout,"-io.out",model,"-sample.mt",
                    "-1","-sample.mv","-1","-sample.vfn","CODE","-sample.vtr","0.1","-classifier","svm",
                    "-classifier.svm.k", "rbf","-classifier.svm.opt","1", *(["-il"] + sp + ["-il"] + indesc), skip=fromstep>10)
...
#Image Classifier (Step 13)
    if os.path.isfile(crop_mask):
        executeStep("ImageClassifier", "otbcli", "CropTypeImageClassifier", buildFolder,
                "-outstat",statistics, "-pixsize", pixsize, "-mode", trm, "-singletile", "false",
                "-bv", "-10000", "model", model, "-mask", cropped_crop_mask, "-out",
                crop_type_map_uncut, *(["-il"] + indesc + ["-sp"] + sp), skip=fromstep>13, rmfiles=[] if keepfiles else [fts])
    else:
        executeStep("ImageClassifier", "otbcli", "CropTypeImageClassifier", buildFolder,
                "-outstat",statistics, "-pixsize", pixsize, "-mode", trm, "-singletile", "false",
                "-bv", "-10000", "-model", model, "-out", crop_type_map_uncut, *(["-il"] + indesc + ["-sp"] + sp), skip=fromstep>13, rmfiles=[] if keepfiles else [fts])
