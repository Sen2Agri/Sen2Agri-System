...
int phenoNormalizedSigmoid(int argc, char * argv[])
{
  if(argc>1)
    {
...

{
  ...
  for(auto test : tdv)
    {
    std::cout << "Testing sample " << test.coord.first << "," << test.coord.second ;
    pheno::VectorType profile(test.y.size());
    pheno::VectorType t(test.t.size());
    ...
    auto pmax = std::max_element(std::begin(profile), std::end(profile)) - std::begin(profile);
    auto pmin = std::min_element(std::begin(profile), std::end(profile)) - std::begin(profile);
    auto vmax = profile[pmax];
    auto vmin = profile[pmin];
    ...
    for(size_t i=0; i<x.size(); i++)
      err += fabs(x[i]-test.x[i])/(fabs(test.x[i])+10e-5);
