...
// Software Guide : BeginCodeSnippet
      ...
      for (otb::ogr::Layer::const_iterator fb=inputLayer.begin(), fe=inputLayer.end(); fb != fe; ++fb)
        {
        otb::ogr::Feature const& inputFeature = *fb;
        destGeometry.addGeometry(inputFeature.GetGeometry());
        char *wkt;
        inputFeature.GetGeometry()->exportToWkt(&wkt);
        std::cout << "Handling feature: " << /*wkt*/ inputFeature.GetGeometry()->exportToJson() << std::endl;
...
struct PushVisitor : boost::static_visitor<>
{
  ...
  template <typename T> void operator()(T & container) const
    ...
    std::cout << m_f.GetName() << " -> " << value << std::endl;
