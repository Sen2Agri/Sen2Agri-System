...
public:
  ...
  typedef struct {
      MeasurementVectorContainer    measContainer;
      GenericMapContainer           mapsContainer;
  } ContainersType;

  typedef std::map<std::string , ContainersType>   FileContainers;

  /** Method to set/get the input list sample */
  void AddInput(const std::string &fileName, const std::string &name,  const MeasurementVectorType& inputVector );
