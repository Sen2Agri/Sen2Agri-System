...
def CheckInput():
	...
	if re.match(r"S2[A|B|C|D]_\w*L1C\w*.SAFE", L1C_file_name) != None:
		if os.path.isdir(L1C_input_path) is False:
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid L1C_input_path {}.".format(L1C_input_path) ,SEN2COR_LOG_FILE_NAME)
			return False
	else:
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid L1C input file name {}.".format(L1C_file_name) ,SEN2COR_LOG_FILE_NAME)
		return False
	docker_input_path = os.path.join("/sen2cor/2.8/input",L1C_file_name)
		
	#--output_dir cheks
	if os.path.isdir(args.output_dir) is False:
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid output directory {}.".format(args.output_dir) ,SEN2COR_LOG_FILE_NAME)
		return False
		...
		if args.sen2cor_exec:
			if os.path.isfile(args.sen2cor_exec):
				...
				if (ProcessorName[0] == "L2A_Process"):
					...
					log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid Sen2Cor ProcessorName {}.".format(ProcessorName[0]) ,SEN2COR_LOG_FILE_NAME)
					return False
			else:
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid Sen2Cor_exe_path {}.".format(args.Sen2Cor_exe_path) ,SEN2COR_LOG_FILE_NAME)
				return False
		else:
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) In local_run mode a path to sen2cor_exec must be given.",SEN2COR_LOG_FILE_NAME)
			return False
		
	#--working_dir cheks
	if args.working_dir:
		if os.path.isdir(args.working_dir) is False:
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid working directory {}.".format(args.working_dir) ,SEN2COR_LOG_FILE_NAME)
			return False
	else:
		#default working dir operations
		default_wrk_dir_name = 'Sen2Cor_wrk'
		default_wrk_dir_path = os.path.join(os.path.dirname(os.path.realpath(__file__)),default_wrk_dir_name)
	...
	if args.resolution:
		...
		for resolution in args.resolution:
			if resolution not in valid_resolution_values:
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid resolution values {}.".format(resolution) ,SEN2COR_LOG_FILE_NAME)
				return False
				
	#--processing_centre checks
	if args.processing_centre:
		if not re.match(r"^[a-zA-Z_]{4}$", args.processing_centre):
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid expression for processing_centre {}.".format(args.processing_centre) ,SEN2COR_LOG_FILE_NAME)
			return False
			
	#--archiving_centre checks
	if args.archiving_centre:
		if not re.match(r"^[a-zA-Z_]{4}$", args.archiving_centre):
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid expression for processing_centre {}.".format(args.archiving_centre) ,SEN2COR_LOG_FILE_NAME)
			return False
			
	#--processing_baseline checks
	if args.processing_baseline:
		if not re.match(r"^[0-9]{2}.[0-9]{2}$", args.processing_baseline):
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid expression for processing_baseline {}.".format(args.processing_baseline) ,SEN2COR_LOG_FILE_NAME)
			return False
	...
	if args.GIP_L2A:
		if os.path.isfile(args.GIP_L2A):
			...
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid GIP_L2A path {}.".format(args.GIP_L2A) ,SEN2COR_LOG_FILE_NAME)
			return False
	...
	if args.GIP_L2A_SC:
		if os.path.isfile(args.GIP_L2A_SC) and os.path.isabs(args.GIP_L2A_SC):
			...
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid GIP_L2A_SC path {}.".format(args.GIP_L2A_SC) ,SEN2COR_LOG_FILE_NAME)
			return False
	...
	if args.GIP_L2A_AC:
		if os.path.isfile(args.GIP_L2A_AC) and os.path.isabs(args.GIP_L2A_AC):
			...
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid GIP_L2A_AC path {}.".format(args.GIP_L2A_AC) ,SEN2COR_LOG_FILE_NAME)
			return False
	...
	if args.GIP_L2A_PB:
		if os.path.isfile(args.GIP_L2A_PB) and os.path.isabs(args.GIP_L2A_PB):
			...
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid GIP_L2A_PB path {}.".format(args.GIP_L2A_PB) ,SEN2COR_LOG_FILE_NAME)
			return False
	...
	if args.mode:
		...
		if args.mode[0] not in valid_modes:
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid mode {}.".format(args.mode) ,SEN2COR_LOG_FILE_NAME)
			return False
	
	#--datastrip checks
	if args.datastrip:
		if os.path.isdir(args.datastrip[0]) == False:
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid datastrip path {}.".format(args.datastrip[0]) ,SEN2COR_LOG_FILE_NAME)
			return False
			
	#--tile checks
	if args.tile:
		if os.path.isdir(args.tile[0]) == False:
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid tile path {}.".format(args.tile[0]) ,SEN2COR_LOG_FILE_NAME)
			return False
			
	#--res_database_dir checks
	if args.res_database_dir:
		if os.path.isdir(args.res_database_dir[0]) == False:
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid res_database_dir path {}.".format(args.res_database_dir[0]) ,SEN2COR_LOG_FILE_NAME)
			return False
			
	#--img_database_dir checks
	if args.img_database_dir:
		if os.path.isdir(args.img_database_dir[0]) == False:
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid img_database_dir path {}.".format(args.img_database_dir[0]) ,SEN2COR_LOG_FILE_NAME)
			return False
	
	if args.tif and args.cog:
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid output format, either cog or tif have to be chosen, not both of them.",SEN2COR_LOG_FILE_NAME)
		return False
	else:
	...
	if args.local_run is False:
		...
		if args.dem_path:
			if os.path.isdir(args.dem_path):
				...
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid dem_path {}.".format(args.dem_path) ,SEN2COR_LOG_FILE_NAME)
				return False
		...
		if args.lc_wb_map_path:
			if os.path.isfile(args.lc_wb_map_path):
				...
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid lc_wb_map_path path {}.".format(args.lc_wb_map_path) ,SEN2COR_LOG_FILE_NAME)
				return False
		...
		if args.lc_lccs_map_path:
			if os.path.isfile(args.lc_lccs_map_path):
				...
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid lc_lccs_map_path path {}.".format(args.lc_lccs_map_path) ,SEN2COR_LOG_FILE_NAME)
				return False
		...
		if args.lc_snow_cond_path:
			if os.path.isdir(args.lc_snow_cond_path):
				...
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Invalid lc_snow_cond_path path {}.".format(args.lc_snow_cond_path) ,SEN2COR_LOG_FILE_NAME)
				return False
...
def CheckOutput():
	...
	L1C_tile = L1C_name_split[5] 
    	L2a_product_pattern = "{}_MSIL2A_{}_N*_{}_{}_*T*.SAFE".format(L1C_satelite, L1C_aquistition, L1C_orbit, L1C_tile)
	...
	#determine if a product with the specified pattern exists
		...
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Multiple L2A products found in the working directory {} for one input l1c product.".format(L2a_products) ,SEN2COR_LOG_FILE_NAME)
		return False, None
	else:
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT find any L2A product in the working directory." ,SEN2COR_LOG_FILE_NAME)
		return False, None
...
def RunSen2Cor():
	...
	if args.local_run == True:
		...
		if args.lc_snow_cond_path:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.lc_snow_cond_path),docker_lc_snow_cond_path))
		if args.lc_lccs_map_path:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.lc_lccs_map_path),docker_lc_lccs_map_path))
		if args.lc_wb_map_path:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.lc_wb_map_path),docker_lc_wb_map_path))
		cmd.append("-v")
		...
		if args.GIP_L2A_AC:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.GIP_L2A_AC),docker_L2A_AC_path))
		if args.GIP_L2A_SC:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.GIP_L2A_SC),docker_L2A_SC_path))
		if args.GIP_L2A_PB:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.GIP_L2A_PB),docker_L2A_PB_path))
		if args.tile:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.tile),docker_tile_path))
		if args.datastrip:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.datastrip),docker_tile_path))
		if args.img_database_dir:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.img_database_dir),docker_img_database_path))
		if args.res_database_dir:
			cmd.append("-v")
			cmd.append("{}:{}".format(os.path.abspath(args.res_database_dir),docker_res_database_path))
		if args.product_id:
			cmd.append("--name")
...
def TranslateToTif(L2A_product_name):
	...
	if len(jp2_files) != 0:
		for jp2 in jp2_files:
			...
			try:
				if run_command(cmd, SEN2COR_LOG_DIR_PATH, SEN2COR_LOG_FILE_NAME, False) != 0:
					log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT translate {} to .tif".format(jp2),SEN2COR_LOG_FILE_NAME)
					return False
				else:
					os.remove(jp2)
			except (KeyboardInterrupt, SystemExit):
				...
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT run the TIFF/COG translation command." ,SEN2COR_LOG_FILE_NAME)
				return False
			...
			#remove jp2 file 
				try:
					...
					log(SEN2COR_LOG_DIR_PATH, "(warning) Can NOT remove file {} from {}.".format(jp2, local_jp2_dir) ,SEN2COR_LOG_FILE_NAME)				
	else:
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT find jp2 files in {}".format(wrk_dir),SEN2COR_LOG_FILE_NAME)
		return False
	...
	if (len(manifest_files) == 1) and (os.path.isfile(manifest_files[0])):
			...
			for dataobject in dataobjects:
				...
				if (file_location_href[-3:] == "jp2"):
					...
					if os.path.isfile(tif_location_path):
						...
						dataobject[0][1].text = hashlib.md5(tif_location_path).hexdigest()
						#update size 
						dataobject[0].attrib["size"] = str(os.path.getsize(tif_location_path))
					else:
						log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT find the translated tif file in the location specified by manifest.safe {}".format(tif_location_path),SEN2COR_LOG_FILE_NAME)
						return False
		except:
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT parse manifest.safe",SEN2COR_LOG_FILE_NAME)
			return False
		...
		try:
			...
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT write the updates to manifest.safe",SEN2COR_LOG_FILE_NAME)
			return False
	else:
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT find the manifest.safe file",SEN2COR_LOG_FILE_NAME)
		return False
	...
	if len(mtd_tl_files) == 1:
		...
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT identify the correct MTD_TL.xml",SEN2COR_LOG_FILE_NAME)
		return False
	try:
		...
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor info) Updated the MTD_TL.xml with tif extensions.",SEN2COR_LOG_FILE_NAME)
	except:
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT parse MTD_TL.xml.",SEN2COR_LOG_FILE_NAME)
		return False
	...
	try:
		...
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT write the updates to MTD_TL.xml",SEN2COR_LOG_FILE_NAME)
		return False
	
	log(SEN2COR_LOG_DIR_PATH, "(sen2cor info) Successful translation from jp2 to tif/cog format.",SEN2COR_LOG_FILE_NAME)
	return True
...
def ConvertPreviews(L2A_product_name):
	...
	if len(jp2_files) != 0:
		for jp2 in jp2_files:
			...
			try:
				if run_command(cmd, SEN2COR_LOG_DIR_PATH, SEN2COR_LOG_FILE_NAME, False) != 0:
					log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT translate {} to .jpeg".format(jp2),SEN2COR_LOG_FILE_NAME)
					return False
				else:
					try:
						os.remove(jp2)
					except:
						log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT remove {} after JPEG translation".format(jp2),SEN2COR_LOG_FILE_NAME)
			except (KeyboardInterrupt, SystemExit):
			 	print("(sen2cor err) Keyboard interrupted")
			 	sys.exit(-1)
			except:
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT run the jpeg translation command".format(jp2),SEN2COR_LOG_FILE_NAME)
				return False						
	else:
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Can NOT find PVI and TCI files in {}".format(wrk_dir),SEN2COR_LOG_FILE_NAME)
		return False
...
def InitLog():	
	...
	if os.path.exists(log_file_path) == False:
		...
		try:
			...
			log_file.write("{}:[{}]:{}\n".format(str(datetime.datetime.now()), os.getpid(), "### Log file created ###"))
			log_file.close()
			print("(sen2cor info) Created a log file {} in dir {}".format(SEN2COR_LOG_FILE_NAME,SEN2COR_LOG_DIR_PATH))
		except:
			print("(sen2cor err) Can NOT create file {} in {}.".format(SEN2COR_LOG_FILE_NAME,SEN2COR_LOG_DIR_PATH))
			return False
	else:
		if os.path.isfile(log_file_path):
			print("(sen2cor info) Using log file {} in dir {}".format(SEN2COR_LOG_FILE_NAME,SEN2COR_LOG_DIR_PATH))
		else:
			print("(sen2cor err) The name of the log file is used by another entity (not a file), aborting")
			return False
...
def Cleanup():
	...
	if  remove_dir(wrk_dir) == False:
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor warning) Can NOT remove file {}".format(wrk_dir))
...
def RunScript():
	try:
		...
		if InitLog() == True:
			...
			log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) NON-NOMINAL initialisation of Sen2Cor script." ,SEN2COR_LOG_FILE_NAME)
			run_script_ok = False
		...
		if run_script_ok == True:
			if CheckInput() == True:
				...
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) NON-NOMINAL input." ,SEN2COR_LOG_FILE_NAME)
				run_script_ok = False
		...
		if run_script_ok == True:	
			if RunSen2Cor() == True:
				...
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) NON-NOMINAL execution of Sen2Cor." ,SEN2COR_LOG_FILE_NAME)
				run_script_ok = False
		...
		if run_script_ok == True:
			...
			if (output_ok == True) and (L2A_product_name != None):
				...
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) NON-NOMINAL output of Sen2Cor processor." ,SEN2COR_LOG_FILE_NAME)
				run_script_ok = False
		
		#convert TCI and PVI files to jpeg format
		if (run_script_ok == True):
			if ConvertPreviews(L2A_product_name) == True:
				print("(sen2cor info) NOMINAL conversion of preview images (TCI, PVI) to jpeg format.")
			else:
				print("(sen2cor err) NON-NOMINAL conversion of preview images (TCI, PVI) to jpeg format.")
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) NON-NOMINAL conversion of preview images (TCI, PVI) to jpeg format." ,SEN2COR_LOG_FILE_NAME)
				run_script_ok = False
		...
		if (run_script_ok == True):
			...
			if translate_ok == True:
				if args.tif:		
					log(SEN2COR_LOG_DIR_PATH, "(sen2cor info) NOMINAL Tiff translation." ,SEN2COR_LOG_FILE_NAME)
				if args.cog:
					log(SEN2COR_LOG_DIR_PATH, "(sen2cor info) NOMINAL COG translation." ,SEN2COR_LOG_FILE_NAME)		
			else:
				print("(sen2cor err) NON-NOMINAL Tiff/COG translation.")
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) NON-NOMINAL Tiff/COG translation." ,SEN2COR_LOG_FILE_NAME)
				run_script_ok = False

		if (run_script_ok == True):
			if CopyOutput(L2A_product_name) == True:
				print("(sen2cor info) NOMIMAL copying of the product from working directory to output directory.")
			else:
				print("(sen2cor err) NON-NOMIMAL copying of the product from working directory to output directory.")
				log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) NON-NOMIMAL copying of the product from working directory to output directory." ,SEN2COR_LOG_FILE_NAME)
				run_script_ok = False
		...
		print("(sen2cor err) Keyboard interrupted.")
		log(SEN2COR_LOG_DIR_PATH, "(sen2cor err) Keyboard interrupted." ,SEN2COR_LOG_FILE_NAME)
		sys.exit(-1)
	except Exception, e:
		print("(sen2cor err): Exception {} encountered".format(e))
	 	log(SEN2COR_LOG_DIR_PATH, "(sen2cor err): Exception {} encountered".format(e) ,SEN2COR_LOG_FILE_NAME)
	 	sys.exit(-1)
...
parser.add_argument('--resolution', required=False, nargs=1,
                    help="Target resolution, can be 10, 20 or 60m. If ommited, only 20  and 10m resolutions will be processed.")
parser.add_argument('--processing_centre', type = str, required=False,
                    help="Processing centre as regular expression, 4 Characters or '_',like	'SGS_'.")
parser.add_argument('--archiving_centre', type = str, required=False,
                    help="Processing centre as regular expression, 4 Characters or '_',like	'SGS_'.")
parser.add_argument('--processing_baseline', type = str, required=False,
                    help="Processing baseline in the format 'dd.dd', where d=[0:9], like '08.15'")
parser.add_argument('--tif', required=False, action="store_true",
                    help="Export raw images in GeoTiff format instead of JPEG-2000.")
parser.add_argument('--cog', required=False, action="store_true",
...
parser.add_argument('--debug', required=False, action="store_true",
                    help="Performs in debug mode.")
parser.add_argument('--sc_only', required=False, action="store_true",
                    help="Performs only	the	scene classification at	60 or 20m resolution.")
parser.add_argument('--cr_only', required=False, action="store_true",
                    help="Performs only	the	creation of the L2A product tree, no processing.")
parser.add_argument('--GIP_L2A', required=False, 
                    help="Select the user GIPP (a filename with full path).")	
parser.add_argument('--GIP_L2A_SC', required=False, 
                    help="Select the scene clarification GIPP (a filename with full path).")
parser.add_argument('--GIP_L2A_AC', required=False, 
                    help="Select the atmospheric correction GIPP (a filename with full path).")
parser.add_argument('--GIP_L2A_PB', required=False, 
                    help="Select the prpcessing baseline GIPP (a filename with full path).")
parser.add_argument('--mode', required=False, type = str, nargs = 1, 
                    help="Mode: generate_datastrip, process_tile.")	
parser.add_argument('--datastrip', required=False, nargs = 1,
                    help="Datastrip folder, process_tile.")
parser.add_argument('--tile', required=False, nargs = 1,
                    help="Tile folder, process_tile.")
parser.add_argument('--img_database_dir', required=False, nargs = 1,
                    help="Tile folder, process_tile.")	
parser.add_argument('--res_database_dir', required=False, nargs = 1,
                    help="Tile folder, process_tile.")						
