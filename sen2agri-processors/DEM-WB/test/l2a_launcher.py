...
class ProcessingContext(object):
        ...
        if len(row) == 3 and row[0] != None and row[2] != None:
            ...
            if parameter == "processor.l2a.s2.implementation":
                ...
                if site != None:
                    ...
                    self.cogTiffs["default"] = (cogTiffs == "1" or cogTiffs.lower() == "true")
            elif parameter == "processor.l2a.optical.compress-tiffs":
                compressTiffs = value
                if site != None:
                    self.cogTiffs[site] = (compressTiffs == "1" or compressTiffs.lower() == "true")
                else:
                    self.cogTiffs["default"] = (compressTiffs == "1" or compressTiffs.lower() == "true") 
                ...
                if site != None:
                    self.removeFreFiles[site] = (removeFreFiles == "1" or removeFreFiles.lower() == "true")
                else:
                    self.removeFreFiles["default"] = (removeFreFiles == "1" or removeFreFiles.lower() == "true");
            elif parameter == "processor.l2a.maja.remove-sre":
                removeSreFiles = value
                if site != None:
                    self.removeSreFiles[site] = (removeSreFiles == "1" or removeSreFiles.lower() == "true")
                else:
                    self.removeSreFiles["default"] = (removeSreFiles == "1" or removeSreFiles.lower() == "true");
            ...
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid database row: {}".format(row), LAUCHER_LOG_FILE_NAME)
    ...
    def get_site_context(self, site_id):
        ...
        if site_id in self.output_path:
            ...
            site_context.base_output_path = site_context.output_path.replace("{processor}", "l2a")
        else:
...
class SiteContext(object):
    ...
    def is_valid(self):
        if not os.path.isfile(self.maja_launcher):
            print("(launcher err) Invalid processing context maja_launcher: {}".format(self.maja_launcher))
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context maja_launcher: {}".format(self.maja_launcher), LAUCHER_LOG_FILE_NAME)
            return False

        if len(self.output_path) == 0:
            print("(launcher err) Invalid processing context output_path: {}.".format(self.output_path))
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context output_path: {}.".format(self.output_path), LAUCHER_LOG_FILE_NAME)
            return False

        if self.num_workers < 1:
            print("(launcher err) Invalid processing context num_workers: {}".format(self.num_workers))
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context num_workers: {}".format(self.num_workers), LAUCHER_LOG_FILE_NAME)
            return False

        if not os.path.isdir(self.swbd_path):
            print("(launcher err) Invalid processing context swbd_path: {}".format(self.swbd_path))
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context swbd_path: {}".format(self.swbd_path), LAUCHER_LOG_FILE_NAME)
            return False

        if not os.path.isdir(self.srtm_path):
            print("(launcher err) Invalid processing context srtm_path: {}".format(self.srtm_path))
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context srtm_path: {}".format(self.srtm_path), LAUCHER_LOG_FILE_NAME)
            return False

        if not os.path.isdir(self.working_dir):
            print("(launcher err) Invalid processing context working_dir: {}".format(self.working_dir))
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context working_dir: {}".format(self.working_dir), LAUCHER_LOG_FILE_NAME)
            return False

        if self.implementation not in ["sen2cor","maja"]:
            print("(launcher err) Invalid processing context implementation: {}".format(self.implementation))
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context implementation: {}".format(self.implementation), LAUCHER_LOG_FILE_NAME)
            return False

        if not os.path.isdir(self.sen2cor_gipp):
            print("(launcher err) Invalid processing context sen2cor_gipp: {}".format(self.sen2cor_gipp))
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context sen2cor_gipp: {}".format(self.sen2cor_gipp), LAUCHER_LOG_FILE_NAME)
            return False

        if not os.path.isdir(self.maja_gipp):
            print("(launcher err) Invalid processing context maja_gipp: {}".format(self.maja_gipp))
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context maja_gipp: {}".format(self.maja_gipp), LAUCHER_LOG_FILE_NAME)
            return False

        if self.removeFreFiles and self.removeSreFiles:
            print("(launcher err) Invalid processing context both removeFreFiles and removeSreFiles are True.")
            log(LAUCHER_LOG_DIR, "(launcher err) Invalid processing context both removeFreFiles and removeSreFiles are True.")
            return False
            ...
            while True:
                ...
                while len(sleeping_workers) > 0:
                    ...
                    if unprocessed_tile != None:
                        ...
                        site_context = processing_context.get_site_context(unprocessed_tile.site_id)
                        ...
                        if tile_validity == False:
                            print("\n(launcher err) <master>: Product {} has invalid tile info.".format(unprocessed_tile.downloader_history_id))
                            log(LAUCHER_LOG_DIR,"(launcher err) <master>: Product {} has invalid tile info.".format(unprocessed_tile.downloader_history_id))
                            db_prerun_update(unprocessed_tile, "Invalid tile information.")
                            continue

                        if site_context_valid == False:
                            print("\n(launcher err) <master>: Product {} has invalid site info.".format(unprocessed_tile.downloader_history_id))
                            log(LAUCHER_LOG_DIR,"(launcher err) <master>: Product {} has invalid site info.".format(unprocessed_tile.downloader_history_id))
                        ...
                        if tile_validity and site_context_valid:
                            ...
                            print("\n(launcher info) <master>: product {} assigned to <worker {}>".format(unprocessed_tile.downloader_history_id, worker_id))
                    else:
                        break
...
class L2aWorker(Thread):
        ...
        try:
            while True:
                ...
                if msg_to_worker == None:
                    ...
                    break
                if msg_to_worker.unprocessed_tile == None or msg_to_worker.site_context == None:
                    print("\n(launcher err) <worker {}>: Either the tile or site context is None".format(self.worker_id, e))
                    os._exit(1)
                else:
                    lin, l2a = self.process_tile(msg_to_worker.unprocessed_tile, msg_to_worker.site_context)
                    self.notify_end_of_tile_processing(lin, l2a)
                    self.worker_q.task_done()
        except Exception, e:
            print("\n(launcher err) <worker {}>: Exception {} encountered".format(self.worker_id, e))
            os._exit(1)
        finally:
    ...
    def process_tile(self, tile, site_context):
        print("\n#################### Tile & Site Info ####################\n")
        print("\n(launcher info) <worker {}>: site_id = {}".format(self.worker_id, tile.site_id))
        print("\n(launcher info) <worker {}>: satellite_id = {}".format(self.worker_id, tile.satellite_id))
        print("\n(launcher info) <worker {}>: orbit_id = {}".format(self.worker_id, tile.orbit_id))
        print("\n(launcher info) <worker {}>: tile_id = {}".format(self.worker_id, tile.tile_id))
        print("\n(launcher info) <worker {}>: downloader_history_id = {}".format(self.worker_id, tile.downloader_history_id))
        print("\n(launcher info) <worker {}>: path = {}".format(self.worker_id, tile.path))
        print("\n(launcher info) <worker {}>: previous_l2a_path = {}".format(self.worker_id, tile.previous_l2a_path)) 
        print("\n(launcher info) <worker {}>: site_short_name = {}".format(self.worker_id, tile.site_short_name))
        print("\n(launcher info) <worker {}>: site_output_path = {}".format(self.worker_id, tile.site_output_path))
        ...
        if tile.satellite_id == LANDSAT_ID:
            ...
            if site_context.implementation == "sen2cor":
                ...
                log(LAUCHER_LOG_DIR, "{}: Aborting processing for site {} because the processor name {} is not recognized".format(worker_id, site_id, threads_context.implementation), LAUCHER_LOG_FILE_NAME)
                return None, None
...
class Tile(object):
    ...
    def is_valid(self):
        if len(self.site_short_name) == 0:
            log(LAUCHER_LOG_DIR, ": Aborting processing for tile {} because site short name {} is incorrect".format(self.tile_id, self.site_short_name), LAUCHER_LOG_FILE_NAME)
            return False
...
class L2aProcessor(object):
    ...
    def check_if_flat_archive(self, output_dir, archive_filename):
        ...
        if len(dir_content) > 1:
            ...
            product_name, file_ext = os.path.splitext(self.path_filename(archive_filename))
            #handle .tar.gz case
    ...
    def unzip(self, output_dir, input_file):
        ...
        try:
            with zipfile.ZipFile(input_file) as zip_archive:
                ...
                return self.check_if_flat_archive(output_dir, self.path_filename(input_file))
        except Exception, e:
            self.launcher_log("Exception when trying to unzip file {}:  {} ".format(input_file, e))
            self.update_rejection_reason("Exception when trying to unzip file {}:  {} ".format(input_file, e))
    ...
    def untar(self, output_dir, input_file):
        ...
        try:
            ...
            self.launcher_log("Exception when trying to untar file {}:  {} ".format(input_file, e))
            self.update_rejection_reason("Exception when trying to untar file {}:  {} ".format(input_file, e))
    ...
    def extract_from_archive_if_needed(self, archive_file):
        ...
        self.launcher_log("This wasn't an archive, so continue as is. Deleting {}".format(extracted_archive_dir))
    ...
    def validate_input_product_dir(self):
        ...
        try:
            os.stat(self.lin.path)
                ...
                self.update_rejection_reason("Can NOT check if product root dir path {} exists or it is a valid symlink. Error was: {}".format(self.lin.path, e.errno))
                return False
        ...
        for root, subdirs, files in os.walk(self.lin.path):
            for subdir in subdirs:
                ...
                try:
                    ...
                    self.update_rejection_reason("Cannot check if dir path {} exists or it is a valid symlink. Error was: {}".format(subdir_path, e.errno))
                    return False
            ...
            for filename in files:
                ...
                try:
                    ...
                    self.update_rejection_reason("Cannot check if file path {} exists or is a valid symlink. Error was: {}".format(subdir_path, e.errno))
                    return False
    ...
    def check_lin(self):
        ...
        self.launcher_log("{}: Starting extract_from_archive_if_needed for tile {}".format(self.context.worker_id, self.lin.tile_id))
        self.lin.was_archived, self.lin.path = self.extract_from_archive_if_needed(self.lin.db_path)
        self.launcher_log("{}: Ended extract_from_archive_if_needed for tile {}".format(self.context.worker_id, self.lin.tile_id))
    ...
    def l2a_setup(self):
        ...
        if lin_basename.startswith("S2"):
            ...
            if lin_basename.find("_L1TP_") > 0 :
                ...
                self.update_rejection_reason("The input product name is wrong - L2A cannot be filled: {}".format(l2a_basename))
                self.launcher_log("The input product name is wrong - L2A cannot be filled: {}".format(l2a_basename))
                return False
        else:
            self.update_rejection_reason("The input product name is wrong: {}".format(l2a_basename))
            self.launcher_log("The input product name is wrong: {}".format(l2a_basename))
            return False
        ...
        if lin_basename.startswith("S2"):
            ...
            if result:
                ...
                self.update_rejection_reason("Can NOT obtain the aquisition date on input product: {}".format(lin_basename))
                self.launcher_log("Can NOT obtain the aquisition date on input product: {}".format(lin_basename))
                return False
        ...
        l2a_output_path = os.path.join(self.lin.site_output_path, acq_year, acq_month, acq_day, l2a_basename)
        if not create_recursive_dirs(l2a_output_path):
            self.update_rejection_reason("Can NOT create the output directory: {}".format(l2a_output_path))
            self.launcher_log("Can NOT create the output directory: {}".format(l2a_output_path))
            return False
    ...
    def update_rejection_reason(self, message):
        ...
        if self.lin.rejection_reason == None:
            ...
            self.lin.rejection_reason = self.lin.rejection_reason + messages_separator + message
...
class Maja(L2aProcessor):
    ...
    def get_l2a_footprint(self):
        ...
        self.update_rejection_reason("Can NOT create the footprint, no FRE tif file exists.")
    ...
    def get_quality_indicators(self):
        ...
        maja_report_file_path = os.path.join(self.l2a.output_path, maja_report_file_name)
        if os.path.isfile(maja_report_file_path):
            try:
                ...
                for message in soup.find_all('message'):
                    ...
                    if re.search("cloud percentage computed is", msg_text, re.IGNORECASE):
                        numbers = [int(s) for s in re.findall(r'\d+', msg_text)]
                        if len(numbers) > 0:
                            self.l2a.cloud_coverage_assessment = numbers[0]
                            cloud_coverage_acq = True
                    if re.search("snow percentage computed is", msg_text, re.IGNORECASE):
                        numbers = [int(s) for s in re.findall(r'\d+', msg_text)]
                        if len(numbers) > 0:
                            self.l2a.snow_ice_percentage = numbers[0]
                            snow_coverage_acq = True 
            except Exception, e:
                self.update_rejection_reason("Exception received when trying to read the MAJA report text from file {}: {}".format(maccs_report_file, e))
                self.l2a_log("Exception received when trying to read the MAJA report text from file {}: {}".format(maccs_report_file, e))
                return Falsezip
        else:
            self.update_rejection_reason("Can NOT find the MAJA report text file: {}".format(maja_report_file_path))
            self.l2a_log("Can NOT find the MAJA report text file: {}".format(maja_report_file_path))
            return False 

        if cloud_coverage_acq == False:
            self.l2a_log("Can NOT extract cloud coverage from {}".format(maja_report_file_path))
            self.update_rejection_reason("Can NOT extract cloud coverage from {}".format(maja_report_file_path))
            return False
        ...
        if (snow_coverage_acq == False) and (self.l2a.cloud_coverage_assessment <= MAX_CLOUD_COVERAGE):
            self.l2a_log("Can NOT extract snow ice coverage from {}".format(maja_report_file_path))
            self.update_rejection_reason("Can NOT extract snow ice coverage from {}".format(maja_report_file_path))
            return False
        return True
            ...
            try:
                ...
                for message in soup.find_all('processing_flags_and_modes'):
                    ...
                    if key == 'Validity_Flag' :
                        ...
                        if value == 'L2NOTV' :
                            self.l2a_log(path, "L2NOTV found in the MAJA JPI file {}. The product will not be retried ... ".format(jpi_file), tile_log_filename)
                            self.lin.should_retry = False
                            self.update_rejection_reason("L2NOTV found in the MAJA JPI file {}. The product will not be retried!".format(jpi_file))
            except Exception, e:
                self.update_rejection_reason("Exception received when trying to read the MAJA JPI from file {}: {}".format(jpi_file, e))
                self.l2a_log("Exception received when trying to read the MAJA JPI from file {}: {}".format(jpi_file, e))
    ...
    def check_l2a_log(self):
        ...
        try:
            ...
            self.update_rejection_reason("Could not read l2a product log at {}".format(tile_log_filepath))
    ...
    def create_mosaic(self):
        ...
        if (len(qkl_files) == 1) and os.path.isfile(qkl_files[0]):
            ...
            try:
                ...
                self.update_rejection_reason("Can NOT copy QKL {} to {}".format(qkl, mosaic))
    ...
    def check_theia_muscate_format(self):
        ...
        for filename in dir_content:
            if os.path.isfile(filename) and re.search("_L2A_T{}_.*ATB.*\.tif$".format(self.lin.tile_id), filename, re.IGNORECASE) is not None:
                atb_files_count += 1
            if os.path.isfile(filename) and re.search("_L2A_T{}_.*FRE.*\.tif$".format(self.lin.tile_id), filename, re.IGNORECASE) is not None:
                fre_files_count += 1
            if os.path.isfile(filename) and re.search("_L2A_T{}_.*SRE.*\.tif$".format(self.lin.tile_id), filename, re.IGNORECASE) is not None:
                sre_files_count += 1
            if os.path.isfile(filename) and re.search("_L2A_T{}_.*MTD.*\.xml$".format(self.lin.tile_id), filename, re.IGNORECASE) is not None:
                mtd_file = True
            if os.path.isfile(filename) and re.search("_L2A_T{}_.*QKL.*\.jpg$".format(self.lin.tile_id), filename, re.IGNORECASE) is not None:
                qkl_file = True
        ...
        if self.lin.satellite_id != satellite_id:
            self.update_rejection_reason("L2A and input product have different satellite ids: {} vs {} .".format(satellite_id, self.lin.satellite_id))
            self.l2a_log("L2A and input product have different satellite ids: {} vs {} .".format(satellite_id, self.lin.satellite_id))
            return False
    ...
    def get_output_format(self):
        ...
        theia_muscate_dir_path = os.path.join(self.l2a.output_path, theia_muscate_dir_pattern)
    ...
    def check_l2a(self, run_script_ok):
        ...
        if self.l2a.output_format == MACCS_OUTPUT_FORMAT:
            ...
            tile_dir_list_path = os.path.join(self.l2a.output_path, tile_dir_list_pattern)
        ...
        if (l2a_found and
            quality_indicators_acquired and
            run_script_ok):
            return True
        else:
    ...
    def run_script(self):
        ...
        script_command = [script_path, "--srtm", self.context.srtm_path, "--swbd", self.context.swbd_path, "--processes-number-dem", "1", "--processes-number-maccs", "1", "--gipp-dir", self.context.gips_path, "--working-dir", self.context.working_dir, "--maccs-launcher", self.context.maja_launcher, "--delete-temp", "True", self.lin.path, self.l2a.output_path]
        ...
        command_return = run_command(script_command, self.l2a.output_path, self.l2a_log_file) 
        if (command_return == 0) and os.path.isdir(self.l2a.output_path):
            return True
        else:
            self.update_rejection_reason("Can NOT run MAJA script, error code: {}.".format(command_return))
            self.l2a_log("Can NOT run MAJA script, error code: {}.".format(command_return))
            return False
    ...
    def manage_prods_status(self, preprocess_succesful, process_succesful, l2a_ok, postprocess_succesful):
        maja_text_to_stop_retrying = ["The number of cloudy pixel is too high", "algorithm processing is stopped", "The dark surface reflectance associated to the value of AOT index min is lower than the dark surface reflectance threshold", "The number of NoData pixel in the output L2 composite product is too high", "PersistentStreamingConditionalStatisticsImageFilter::Synthetize.No pixel is valid. Return null statistics"]
        
        if ((preprocess_succesful == True) and
            (process_succesful == True) and
            (l2a_ok == True) and 
    ...
    def run(self):
        ...
        print("\n(launcher info) <worker {}>: NOMINAL pre-processing = {}".format(self.context.worker_id, preprocess_succesful))
        self.l2a_log("NOMINAL pre-processing = {}".format(preprocess_succesful))
        
        #processing
        if preprocess_succesful:
            process_succesful = self.run_script()
        print("\n(launcher info) <worker {}>: NOMINAL processing = {}".format(self.context.worker_id, process_succesful))
        self.l2a_log("NOMINAL processing = {}".format(process_succesful))

        #processing checks
        l2a_ok = self.check_l2a(process_succesful)
        print("\n(launcher info) <worker {}>: NOMINAL L2a product = {}".format(self.context.worker_id, l2a_ok))
        ...
        if l2a_ok and self.get_l2a_footprint() and self.create_mosaic():
            self.postprocess()
            postprocess_succesful = True
        print("\n(launcher info) <worker {}>: NOMINAL post-processing = {}".format(self.context.worker_id, postprocess_succesful))
        self.l2a_log("NOMINAL post-processing = {}".format(postprocess_succesful))

        self.manage_prods_status(preprocess_succesful, process_succesful, l2a_ok, postprocess_succesful)
...
class Sen2Cor(L2aProcessor):
    ...
    def get_l2a_footprint(self):
        ...
        if len(footprint_files) > 0:
            ...
            self.update_rejection_reason("Can NOT create the footprint, no B02_10m/20m/60m.tif file exists.")
            self.l2a_log("Can NOT create the footprint, no B02_10m/20m/60m.tif file exists.")
            return False
    ...
    def get_quality_indicators(self):
        ...
        if os.path.isfile(mtd_path):
            try:
                ...
                cloud_coverage_assessment = float(quality_indicators_info.findtext("Cloud_Coverage_Assessment"))
                image_content_qi = quality_indicators_info.find("Image_Content_QI")
                nodata_pixel_percentage = float(image_content_qi.findtext("NODATA_PIXEL_PERCENTAGE"))
                saturated_defective_pixel_percentage = float(image_content_qi.findtext("SATURATED_DEFECTIVE_PIXEL_PERCENTAGE"))
                dark_features_percentage = float(image_content_qi.findtext("DARK_FEATURES_PERCENTAGE"))
                cloud_shadow_percentage = float(image_content_qi.findtext("CLOUD_SHADOW_PERCENTAGE"))
                snow_ice_percentage = float(image_content_qi.findtext("SNOW_ICE_PERCENTAGE"))
                valid_pixels_percentage = ((100 - nodata_pixel_percentage) / 100.0 * ( 100 - cloud_coverage_assessment - saturated_defective_pixel_percentage - dark_features_percentage - cloud_shadow_percentage - snow_ice_percentage))
            except:
                self.update_rejection_reason("Can NOT parse {} for quality indicators extraction.".format(mtd_path))
                self.l2a_log("Can NOT parse {} for quality indicators extraction.".format(mtd_path))
                return False  
        else:
            self.update_rejection_reason("Can NOT find MTD_MSIL2A.xml file in location.".format(mtd_path))
            self.l2a_log("Can NOT find MTD_MSIL2A.xml file in location.".format(mtd_path))
            return False
    ...
    def get_rejection_reason(self):
        log_path = os.path.join(self.context.processor_log_dir, self.context.processor_log_file)
        if os.path.isfile(log_path):
            try:
                ...
                rejection_reason = "Can NOT read from sen2cor log file {} to get the rejection reason".format(log_path)
                self.l2a_log("Can NOT read from sen2cor log file {} to get the rejection reason".format(log_path))
        else:
            rejection_reason = "Can NOT find sen2cor log file {} to get the rejection reason".format(log_path)
            self.l2a_log("Can NOT find sen2cor log file {} to get the rejection reason".format(log_path))
            
        return rejection_reason 

    def create_mosaic(self):
        # Copy the PVI file to the output path as mosaic.jpg
        pvi_pattern = "GRANULE/L2A*_T{}_*/QI_DATA/*PVI.jpg".format(self.l2a.processed_tiles[0])
        ...
        if len(pvi_files) == 1 and os.path.isfile(pvi_files[0]):
            ...
            try:
                ...
                self.update_rejection_reason("Can NOT copy PVI {} to {}".format(pvi_files[0], mosaic))
                self.l2a_log("Can NOT copy PVI {} to {}".format(pvi_files[0], mosaic))
                return False
        else:
            self.update_rejection_reason("Can NOT indentify a PVI image to create mosaic.jpg .")
            self.l2a_log("Can NOT indentify a PVI image to create mosaic.jpg .")
            return False

    def check_l2a(self):
        l2a_product_name_pattern = "S2[A|B|C|D]_MSIL2A_*_T{}_*.SAFE".format(self.lin.tile_id)
        ...
        if len(l2a_products) == 1:
            ...
            self.update_rejection_reason("No product or multiple L2A products are present in the output directory.")
            self.l2a_log("No product or multiple L2A products are present in the output directory.")
            return False
    ...
    def run_script(self):
        ...
        if os.path.isfile(gipp_l2a_path):
            ...
            l2a_log("(launcher warning) Can NOT find L2A_GIPP.xml at {}, proceeding with default GIPP".format(gipp_l2a_path))
        
        lc_snow_cond_path = os.path.join(self.context.gips_path, "ESACCI-LC-L4-Snow-Cond-500m-P13Y7D-2000-2012-v2.0")
        if os.path.isdir(lc_snow_cond_path):
            script_command.append("--lc_snow_cond_path")
            script_command.append(lc_snow_cond_path)
        else:
            l2a_log("(launcher warning) Can NOT find ESACCI-LC-L4-Snow-Cond-500m-P13Y7D-2000-2012-v2.0 at {}, proceeding with default GIPP".format(lc_snow_cond_path))
        
        lc_lccs_map_path = os.path.join(self.context.gips_path, "ESACCI-LC-L4-LCCS-Map-300m-P1Y-2015-v2.0.7.tif")
        if os.path.isfile(lc_lccs_map_path):
            script_command.append("--lc_lccs_map_path")
            script_command.append(lc_lccs_map_path)
        else:
            l2a_log("(launcher warning) Can NOT find ESACCI-LC-L4-LCCS-Map-300m-P1Y-2015-v2.0.7.tif at {}, proceeding with default GIPP".format(lc_lccs_map_path))

        lc_wb_map_path = os.path.join(self.context.gips_path, "ESACCI-LC-L4-WB-Map-150m-P13Y-2000-v4.0.tif")
        if os.path.isfile(lc_wb_map_path):
            script_command.append("--lc_wb_map_path")
            script_command.append(lc_wb_map_path)
        else:
            l2a_log("(launcher warning) Can NOT find ESACCI-LC-L4-WB-Map-150m-P13Y-2000-v4.0.tif at {}, proceeding with default GIPP".format(lc_wb_map_path))

        wrk_dir = os.path.join(self.context.working_dir, self.l2a.basename)
        if not os.path.exists(wrk_dir):
            if create_recursive_dirs(wrk_dir) == False:
                self.update_rejection_reason("Can NOT create wrk dir {}".format(wrk_dir))
        ...
        command_return = run_command(script_command, self.l2a.output_path, self.l2a_log_file) 
        if ((command_return == 0) and 
            os.path.isdir(self.l2a.output_path)):
            return True
        else:
            reason = self.get_rejection_reason()
            self.update_rejection_reason(reason)
            self.l2a_log("Sen2Cor returned error code: {} due to : {}.".format(command_return, reason))
            return False
        ...
        if ((preprocess_succesful == True) and
            ...
            if quality_indicators_acquired:  
                if (self.l2a.valid_pixels_percentage < MIN_VALID_PIXELS_THRESHOLD):
                    self.lin.should_retry = False
                    self.update_rejection_reason("The valid pixels percentage is {} which is less that the threshold of {}%".format(self.l2a.valid_pixels_percentage, MIN_VALID_PIXELS_THRESHOLD))
                    self.l2a_log("The valid pixels percentage is {} which is less that the threshold of {}%".format(self.l2a.valid_pixels_percentage, MIN_VALID_PIXELS_THRESHOLD))              
                    if DEBUG == False:
                        remove_dir(self.l2a.product_path)

            else:
                self.lin.should_retry = True
                self.update_rejection_reason("Can NOT extract pixel quality indicators from MTD_MSIL2A.xml for: cloud, snow, valid pixels coverage assesment")
                snow_coverage = None
                cloud_coverage = None
        else:
            self.lin.processing_status = DATABASE_DOWNLOADER_STATUS_PROCESSING_ERR_VALUE
    ...
    def run(self):
        ...
        if self.check_lin() and self.l2a_setup():
            preprocess_succesful = True
        print("\n(launcher info) <worker {}>: NOMINAL  pre-processing = {}".format(self.context.worker_id, preprocess_succesful))
        self.l2a_log("NOMINAL  pre-processing = {}".format(self.context.worker_id, preprocess_succesful))
        
        if preprocess_succesful and self.run_script():
            process_succesful = True
        print("\n(launcher info) <worker {}>: NOMINAL Sen2Cor processing = {}".format(self.context.worker_id, process_succesful))
        self.l2a_log("NOMINAL Sen2Cor processing = {}".format(self.context.worker_id, process_succesful))
        ...
        if l2a_ok and self.get_l2a_footprint() and self.create_mosaic():
            postprocess_succesful = True
        print("\n(launcher info) <worker {}>: NOMINAL post-processing = {}".format(self.context.worker_id, postprocess_succesful))
        self.l2a_log("NOMINAL post-processing = {}".format(self.context.worker_id, postprocess_succesful))
...
def db_update(db_func):
        def wrapper_db_update(*args, **kwargs):
            ...
            if not products_db.database_connect():
                log(LAUCHER_LOG_DIR ,"{}: Database connection failed upon updating the database.".format(threading.currentThread().getName()), LAUCHER_LOG_FILE_NAME)
                return db_updated
            while True:
                try:
                    db_func(*args, **kwargs)
                    ...
                    if (e.pgcode in (SERIALIZATION_FAILURE, DEADLOCK_DETECTED,) and nb_retries > 0):
                        log(LAUCHER_LOG_DIR,"{}: Exception {} when trying to update db: SERIALIZATION failure".format(threading.currentThread().getName(), e.pgcode), LAUCHER_LOG_FILE_NAME)
                        time.sleep(random.uniform(0,max_sleep))
                        max_sleep *= 2
                        nb_retries -= 1
                        continue
                    log(LAUCHER_LOG_DIR,"{}: Exception {} when trying to update db".format(threading.currentThread().getName(), e.pgcode), LAUCHER_LOG_FILE_NAME)
                    raise
                except Exception as e:
                    products_db.conn.rollback()
                    log(LAUCHER_LOG_DIR,"{}: Exception {} when trying to update db".format(threading.currentThread().getName(), e), LAUCHER_LOG_FILE_NAME)
                    raise
                else:
                    log(LAUCHER_LOG_DIR,"{}: NOMINAL db update".format(threading.currentThread().getName()), LAUCHER_LOG_FILE_NAME)
                    db_updated = True
                    break
                finally:
        ...
        def wrapper_db_fetch(*args, **kwargs):
            ...
            while True:
                ...
                try:
                    ...
                    products_db.conn.rollback()
                    if (e.pgcode in (SERIALIZATION_FAILURE, DEADLOCK_DETECTED,) and nb_retries > 0):
                        log(LAUCHER_LOG_DIR,"{}: Exception {} when trying to fetch from db: SERIALIZATION failure".format(threading.currentThread().getName(), e.pgcode), LAUCHER_LOG_FILE_NAME)
                        time.sleep(random.uniform(0,max_sleep))
                        max_sleep *= 2
                        nb_retries -= 1
                        continue
                    log(LAUCHER_LOG_DIR,"{}: Exception {} when trying to fetch from db".format(threading.currentThread().getName(), e.pgcode), LAUCHER_LOG_FILE_NAME)
                    raise
                except Exception as e:
                    products_db.conn.rollback()
                    log(LAUCHER_LOG_DIR,"{}: Exception {} when trying to fetch from db".format(threading.currentThread().getName(), e), LAUCHER_LOG_FILE_NAME)
                    raise
                else:
                    log(LAUCHER_LOG_DIR,"{}: NOMINAL db fetch".format(threading.currentThread().getName()), LAUCHER_LOG_FILE_NAME)
                    break
                finally:
                    products_db.database_disconnect()
...
def db_postrun_update(input_prod, l2a_prod):        
        ...
        if reason != None:
            products_db.cursor.execute("""SELECT * FROM sp_mark_l1_tile_failed(%(downloader_history_id)s :: integer,
                                                                                        %(tile_id)s, 
                                                                                        %(reason)s, 
            ...
            products_db.cursor.execute("""SELECT * FROM sp_mark_l1_tile_done(%(downloader_history_id)s :: integer,
                                                                                    %(tile_id)s,
                                                                                    %(cloud_coverage)s :: integer,
                                                                                    %(snow_coverage)s :: integer);""",
                                    {
        ...
        products_db.cursor.execute("""update downloader_history set status_id = %(status_id)s :: smallint where id=%(l1c_id)s :: integer;""",
                                 {
                                     "status_id": processing_status,
                                     "l1c_id": downloader_product_id
                                 })

        #update product table
        if (reason == None) and (processing_status == DATABASE_DOWNLOADER_STATUS_PROCESSED_VALUE):
            products_db.cursor.execute("""select * from sp_insert_product(%(product_type_id)s :: smallint,
...
def db_get_site_short_name(site_id):
    products_db.cursor.execute("set transaction isolation level serializable;")
    products_db.cursor.execute("select short_name from site where id={}".format(site_id))
...
parser = argparse.ArgumentParser(
    description="Launcher for MAJA/Sen2Cor script")
...
if not config.loadConfig(args.config):
    log(LAUCHER_LOG_DIR, "Could not load the config from configuration file", LAUCHER_LOG_FILE_NAME)
    sys.exit(-1)
...
if processing_context == None:
    log(LAUCHER_LOG_DIR, "Could not load the config from database", LAUCHER_LOG_FILE_NAME)
    sys.exit(-1)
...
#create working dir
if not os.path.isdir(default_site_context.working_dir) and not create_recursive_dirs(default_site_context.working_dir):
    log(LAUCHER_LOG_DIR, "Could not create the work base directory {}".format(default_site_context.working_dir), LAUCHER_LOG_FILE_NAME)
    sys.exit(-1)
