...
#include "SpectralFeatures.hxx"

typedef otb::ImageFileReader<ImageType>                                         ReaderType;
typedef SpectralFeaturesFunctor<ImageType::PixelType>                           SpectralFeaturesFunctorType;
typedef BinaryFunctorImageFilterWithNBands<SpectralFeaturesFunctorType>         BinaryFunctorImageFilterWithNBandsType;
...
//  Software Guide : BeginCodeSnippet
class SpectralFeatures : public Application
//  Software Guide : EndCodeSnippet
{
public:
  //  Software Guide : BeginLatex
...
;
  ...
  itkTypeMacro(SpectralFeatures, otb::Application)
;
  //  Software Guide : EndCodeSnippet
...
private:
  ...
  void DoInit()
  {
    ...
    AddParameter(ParameterType_InputImage, "tf", "Temporal features");
    ...
    SetDocExampleParameterValue("tf", "tf.tif");
  ...
  void DoExecute()
  {
      ...
      m_TFReader->SetFileName(GetParameterString("tf"));
      m_TFReader->UpdateOutputInformation();

      // connect the functor based filter
      m_filter->SetNumberOfOutputBands(m_bands * m_TFReader->GetOutput()->GetNumberOfComponentsPerPixel());
      m_filter->SetFunctor(SpectralFeaturesFunctorType(m_bands));

      m_filter->SetInput(0, m_TSReader->GetOutput());
      m_filter->SetInput(1, m_TFReader->GetOutput());
  ...
  // The number of bands per input image
  int m_bands;

  ReaderType::Pointer                                m_TFReader;
...
// Software Guide : EndLatex
//  Software Guide :BeginCodeSnippet
OTB_APPLICATION_EXPORT(otb::Wrapper::SpectralFeatures)
//  Software Guide :EndCodeSnippet
