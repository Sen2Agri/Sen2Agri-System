...
void LaiRetrievalHandler::CreateNewProductInJobTasks(QList<TaskToSubmit> &outAllTasksList,
                                                     int nbProducts)
{
    ...
    // NOTE: In this moment, the products in loop are not executed in parallel. To do this, the if(i
    // > 0) below
    //      should be removed but in this case, the time-series-builders should wait for all the
    //      monodate images
    int i;
    for (i = 0; i < nbProducts; i++) {
        if (i > 0) {
            // update the ndvi-rvi-extractor with the reference of the previous
            // bv-err-image-inversion
            int nNdviRviExtractorIdx = i * TasksNoPerProduct;
            int nPrevBvErrImgInvIdx = (i - 1) * TasksNoPerProduct + (TasksNoPerProduct - 1);
            outAllTasksList[nNdviRviExtractorIdx].parentTasks.append(
                outAllTasksList[nPrevBvErrImgInvIdx - 1]);
            outAllTasksList[nNdviRviExtractorIdx].parentTasks.append(
                outAllTasksList[nPrevBvErrImgInvIdx]);
        }
    ...
    int nPrevBvErrImgInvIdx = (i - 1) * TasksNoPerProduct + (TasksNoPerProduct - 1);
    outAllTasksList[m_nTimeSeriesBuilderIdx].parentTasks.append(
        outAllTasksList[nPrevBvErrImgInvIdx - 1]);
    outAllTasksList[m_nTimeSeriesBuilderIdx].parentTasks.append(
        outAllTasksList[nPrevBvErrImgInvIdx]);
    ...
    outAllTasksList[m_nErrTimeSeriesBuilderIdx].parentTasks.append(
        outAllTasksList[nPrevBvErrImgInvIdx - 1]);
    outAllTasksList[m_nErrTimeSeriesBuilderIdx].parentTasks.append(
        outAllTasksList[nPrevBvErrImgInvIdx]);
    ...
    outAllTasksList[m_nLaiMskFlgsTimeSeriesBuilderIdx].parentTasks.append(
        outAllTasksList[nPrevBvErrImgInvIdx - 1]);
    outAllTasksList[m_nLaiMskFlgsTimeSeriesBuilderIdx].parentTasks.append(
        outAllTasksList[nPrevBvErrImgInvIdx]);

    // profile-reprocessing -> time-series-builder AND err-time-series-builder AND
    // lai-msk-flags-time-series-builder
    m_nProfileReprocessingIdx = m_nLaiMskFlgsTimeSeriesBuilderIdx + 1;
    outAllTasksList[m_nProfileReprocessingIdx].parentTasks.append(
        outAllTasksList[m_nTimeSeriesBuilderIdx]);
    outAllTasksList[m_nProfileReprocessingIdx].parentTasks.append(
        outAllTasksList[m_nErrTimeSeriesBuilderIdx]);
    outAllTasksList[m_nProfileReprocessingIdx].parentTasks.append(
        outAllTasksList[m_nLaiMskFlgsTimeSeriesBuilderIdx]);

    // reprocessed-profile-splitter -> profile-reprocessing
    m_nReprocessedProfileSplitterIdx = m_nProfileReprocessingIdx + 1;
    outAllTasksList[m_nReprocessedProfileSplitterIdx].parentTasks.append(
        outAllTasksList[m_nProfileReprocessingIdx]);

    // fitted-profile-reprocessing -> time-series-builder AND err-time-series-builder AND
    // lai-msk-flags-time-series-builder
    m_nFittedProfileReprocessingIdx = m_nReprocessedProfileSplitterIdx + 1;
    outAllTasksList[m_nFittedProfileReprocessingIdx].parentTasks.append(
        outAllTasksList[m_nTimeSeriesBuilderIdx]);
    outAllTasksList[m_nFittedProfileReprocessingIdx].parentTasks.append(
        outAllTasksList[m_nErrTimeSeriesBuilderIdx]);
    outAllTasksList[m_nFittedProfileReprocessingIdx].parentTasks.append(
    ...
    outAllTasksList[m_nFittedProfileReprocessingSplitterIdx].parentTasks.append(
        outAllTasksList[m_nFittedProfileReprocessingIdx]);
    ...
    outAllTasksList[m_nProductFormatterIdx].parentTasks.append(
        outAllTasksList[m_nReprocessedProfileSplitterIdx]);
    outAllTasksList[m_nProductFormatterIdx].parentTasks.append(
        outAllTasksList[m_nFittedProfileReprocessingSplitterIdx]);
}

void LaiRetrievalHandler::HandleNewProductInJob(EventProcessingContext &ctx,
                                                int jobId,
                                                ...
                                                const QStringList &listProducts)
{
    ...
    std::map<QString, QString> configParameters =
        ctx.GetJobConfigurationParameters(jobId, "processor.l3b.lai.");
    ...
    for (i = 0; i < listProducts.size(); i++) {
        ...
        ctx.SubmitTasks(jobId,
                        { ndviRviExtractorTask, getLaiRetrievalModelTask, bvImageInversionTask,
                          bvErrImageInversionTask, genMonoDateMskFagsTask });
        ...
        const auto &monoDateErrFileName =
            bvErrImageInversionTask.GetFilePath("LAI_mono_date_ERR_img.tif");
        const auto &monoDateMskFlgsFileName =
            genMonoDateMskFagsTask.GetFilePath("LAI_mono_date_msk_flgs_img.tif");
        ...
        QStringList ndviRviExtractionArgs =
            GetNdviRviExtractionArgs(inputProduct, ftsFile, resolution);
        QStringList getLaiModelArgs = GetLaiModelExtractorArgs(
            inputProduct, modelsList, errModelsList, modelFileName, errModelFileName);
        QStringList bvImageInvArgs = GetBvImageInvArgs(ftsFile, modelFileName, monoDateLaiFileName);
        QStringList bvErrImageInvArgs =
            GetBvErrImageInvArgs(ftsFile, errModelFileName, monoDateErrFileName);
        QStringList genMonoDateMskFagsArgs =
            GetMonoDateMskFagsArgs(inputProduct, monoDateMskFlgsFileName);
        ...
        steps.append(genMonoDateMskFagsTask.CreateStep("GenerateLaiMonoDateMaskFlags",
                                                       genMonoDateMskFagsArgs));
    }
    ...
    TaskToSubmit &fittedProfileReprocSplitTask =
        allTasksList[m_nFittedProfileReprocessingSplitterIdx];
    TaskToSubmit &productFormatterTask = allTasksList[m_nProductFormatterIdx];

    ctx.SubmitTasks(jobId, { imgTimeSeriesBuilderTask, errTimeSeriesBuilderTask,
                             mskFlagsTimeSeriesBuilderTask, profileReprocTask,
                             profileReprocSplitTask, fittedProfileReprocTask,
                             fittedProfileReprocSplitTask, productFormatterTask });

    const auto &allLaiTimeSeriesFileName =
        imgTimeSeriesBuilderTask.GetFilePath("LAI_time_series.tif");
    const auto &allErrTimeSeriesFileName =
        errTimeSeriesBuilderTask.GetFilePath("Err_time_series.tif");
    const auto &allMskFlagsTimeSeriesFileName =
        mskFlagsTimeSeriesBuilderTask.GetFilePath("Mask_Flags_time_series.tif");

    const auto &reprocTimeSeriesFileName =
        profileReprocTask.GetFilePath("ReprocessedTimeSeries.tif");
    const auto &fittedTimeSeriesFileName =
        fittedProfileReprocTask.GetFilePath("FittedTimeSeries.tif");

    const auto &fittedFileListFileName =
        fittedProfileReprocSplitTask.GetFilePath("FittedFilesList.txt");
    const auto &fittedFlagsFileListFileName =
        fittedProfileReprocSplitTask.GetFilePath("FittedFlagsFilesList.txt");

    const auto &reprocFileListFileName =
        profileReprocSplitTask.GetFilePath("ReprocessedFilesList.txt");
    const auto &reprocFlagsFileListFileName =
        profileReprocSplitTask.GetFilePath("ReprocessedFlagsFilesList.txt");

    QStringList timeSeriesBuilderArgs =
        GetTimeSeriesBuilderArgs(monoDateLaiFileNames, allLaiTimeSeriesFileName);
    QStringList errTimeSeriesBuilderArgs =
        GetErrTimeSeriesBuilderArgs(monoDateErrLaiFileNames, allErrTimeSeriesFileName);
    QStringList mskFlagsTimeSeriesBuilderArgs = GetMskFlagsTimeSeriesBuilderArgs(
        monoDateMskFlagsLaiFileNames, allMskFlagsTimeSeriesFileName);

    QStringList profileReprocessingArgs = GetProfileReprocessingArgs(
        configParameters, allLaiTimeSeriesFileName, allErrTimeSeriesFileName,
        allMskFlagsTimeSeriesFileName, reprocTimeSeriesFileName, listProducts);
    QStringList reprocProfileSplitterArgs =
        GetReprocProfileSplitterArgs(reprocTimeSeriesFileName, reprocFileListFileName,
                                     reprocFlagsFileListFileName, listProducts);
    QStringList fittedProfileReprocArgs = GetFittedProfileReprocArgs(
        allLaiTimeSeriesFileName, allErrTimeSeriesFileName, allMskFlagsTimeSeriesFileName,
        fittedTimeSeriesFileName, listProducts);
    QStringList fittedProfileReprocSplitterArgs =
        GetFittedProfileReprocSplitterArgs(fittedTimeSeriesFileName, fittedFileListFileName,
                                           fittedFlagsFileListFileName, listProducts);

    // Get the tile ID from the product XML name. We extract it from the first product in the list
    // as all
    // producs should be for the same tile
    QString tileId = ProcessorHandlerHelper::GetTileId(listProducts);
    QStringList productFormatterArgs = GetProductFormatterArgs(
        productFormatterTask, configParameters, listProducts, ndviFileNames, monoDateLaiFileNames,
        monoDateErrLaiFileNames, monoDateMskFlagsLaiFileNames, fittedFileListFileName,
        fittedFlagsFileListFileName, reprocFileListFileName, reprocFlagsFileListFileName, tileId);

    // add these steps to the steps list to be submitted
    steps.append(imgTimeSeriesBuilderTask.CreateStep("TimeSeriesBuilder", timeSeriesBuilderArgs));
    steps.append(
        errTimeSeriesBuilderTask.CreateStep("TimeSeriesBuilder", errTimeSeriesBuilderArgs));
    steps.append(mskFlagsTimeSeriesBuilderTask.CreateStep("TimeSeriesBuilder",
                                                          mskFlagsTimeSeriesBuilderArgs));
    steps.append(profileReprocTask.CreateStep("ProfileReprocessing", profileReprocessingArgs));
    steps.append(
        profileReprocSplitTask.CreateStep("ReprocessedProfileSplitter", reprocProfileSplitterArgs));
    steps.append(
        fittedProfileReprocTask.CreateStep("ProfileReprocessing", fittedProfileReprocArgs));
    steps.append(fittedProfileReprocSplitTask.CreateStep("ReprocessedProfileSplitter2",
                                                         fittedProfileReprocSplitterArgs));
                                                  ...
                                                  const QStringList &listProducts)
{
    ...
    try {
        ...
        executionInfosFile << "    <bwr_for_algo_local_online_retrieval>" << bwr.toStdString()
                           << "</bwr_for_algo_local_online_retrieval>" << std::endl;
        executionInfosFile << "    <fwr_for_algo_local_online_retrieval>" << fwr.toStdString()
                           << "</fwr_for_algo_local_online_retrieval>" << std::endl;
        executionInfosFile << "  </ProfileReprocessing_parameters>" << std::endl;
...
void LaiRetrievalHandler::GetModelFileList(QStringList &outListModels,
...
QStringList LaiRetrievalHandler::GetNdviRviExtractionArgs(const QString &inputProduct,
                                                          const QString &ftsFile,
                                                          const QString &resolution)
{
    return { "NdviRviExtraction2", "-xml", inputProduct, "-fts", ftsFile, "-outres", resolution };
}

QStringList LaiRetrievalHandler::GetLaiModelExtractorArgs(const QString &inputProduct,
                                                          const QStringList &modelsList,
                                                          const QStringList &errModelsList,
                                                          const QString &modelFileName,
                                                          const QString &errModelFileName)
{
    QStringList laiModelExtractorArgs = { "GetLaiRetrievalModel", "-xml",
                                          inputProduct,           "-outm",
                                          modelFileName,          "-outerr",
                                          errModelFileName,       "-ilmodels" };
...
QStringList LaiRetrievalHandler::GetBvImageInvArgs(const QString &ftsFile,
                                                   const QString &modelFileName,
                                                   const QString &monoDateLaiFileName)
{
    return { "BVImageInversion", "-in", ftsFile, "-modelfile", modelFileName, "-out",
             monoDateLaiFileName };
}

QStringList LaiRetrievalHandler::GetBvErrImageInvArgs(const QString &ftsFile,
                                                      const QString &errModelFileName,
                                                      const QString &monoDateErrFileName)
{
    return { "BVImageInversion", "-in", ftsFile, "-modelfile", errModelFileName, "-out",
             monoDateErrFileName };
}

QStringList LaiRetrievalHandler::GetMonoDateMskFagsArgs(const QString &inputProduct,
                                                        const QString &monoDateMskFlgsFileName)
{
    return { "GenerateLaiMonoDateMaskFlags", "-inxml", inputProduct, "-out",
             monoDateMskFlgsFileName };
}

QStringList LaiRetrievalHandler::GetTimeSeriesBuilderArgs(const QStringList &monoDateLaiFileNames,
                                                          const QString &allLaiTimeSeriesFileName)
{
    QStringList timeSeriesBuilderArgs = { "TimeSeriesBuilder", "-out", allLaiTimeSeriesFileName,
                                          "-il" };
...
QStringList
LaiRetrievalHandler::GetErrTimeSeriesBuilderArgs(const QStringList &monoDateErrLaiFileNames,
                                                 const QString &allErrTimeSeriesFileName)
{
    QStringList timeSeriesBuilderArgs = { "TimeSeriesBuilder", "-out", allErrTimeSeriesFileName,
                                          "-il" };
...
QStringList LaiRetrievalHandler::GetMskFlagsTimeSeriesBuilderArgs(
    const QStringList &monoDateMskFlagsLaiFileNames, const QString &allMskFlagsTimeSeriesFileName)
{
    QStringList timeSeriesBuilderArgs = { "TimeSeriesBuilder", "-out",
                                          allMskFlagsTimeSeriesFileName, "-il" };
...
QStringList
LaiRetrievalHandler::GetProfileReprocessingArgs(std::map<QString, QString> configParameters,
                                                const QString &allLaiTimeSeriesFileName,
                                                const QString &allErrTimeSeriesFileName,
                                                const QString &allMsksTimeSeriesFileName,
                                                const QString &reprocTimeSeriesFileName,
                                                const QStringList &listProducts)
{
    ...
    QStringList profileReprocessingArgs = { "ProfileReprocessing",
                                            "-lai",
                                            allLaiTimeSeriesFileName,
                                            "-err",
                                            allErrTimeSeriesFileName,
                                            "-msks",
                                            allMsksTimeSeriesFileName,
                                            "-opf",
                                            reprocTimeSeriesFileName,
                                            "-algo",
                                            "local",
                                            "-algo.local.bwr",
                                            localWindowBwr,
...
QStringList
LaiRetrievalHandler::GetReprocProfileSplitterArgs(const QString &reprocTimeSeriesFileName,
                                                  ...
                                                  const QStringList &allXmlsFileName)
{
    QStringList args = { "ReprocessedProfileSplitter2",
                         ...
                         reprocTimeSeriesFileName,
                         "-outrlist",
                         reprocFileListFileName,
                         "-outflist",
                         reprocFlagsFileListFileName,
                         "-compress",
                         "1",
                         "-ilxml" };
...
QStringList
LaiRetrievalHandler::GetFittedProfileReprocArgs(const QString &allLaiTimeSeriesFileName,
                                                const QString &allErrTimeSeriesFileName,
                                                const QString &allMsksTimeSeriesFileName,
                                                const QString &fittedTimeSeriesFileName,
                                                const QStringList &listProducts)
{
...
QStringList
LaiRetrievalHandler::GetFittedProfileReprocSplitterArgs(const QString &fittedTimeSeriesFileName,
                                                        const QString &fittedFileListFileName,
                                                        ...
                                                        const QStringList &allXmlsFileName)
{
    QStringList args = { "ReprocessedProfileSplitter2",
                         ...
                         fittedTimeSeriesFileName,
                         "-outrlist",
                         fittedFileListFileName,
                         "-outflist",
                         fittedFlagsFileListFileName,
                         "-compress",
                         "1",
                         "-ilxml" };
    args += allXmlsFileName;
...
QStringList
LaiRetrievalHandler::GetProductFormatterArgs(TaskToSubmit &productFormatterTask,
                                             std::map<QString, QString> configParameters,
                                             const QStringList &listProducts,
                                             const QStringList &listNdvis,
                                             const QStringList &listLaiMonoDate,
                                             const QStringList &listLaiMonoDateErr,
                                             const QStringList &listLaiMonoDateFlgs,
                                             const QString &fileLaiReproc,
                                             const QString &fileLaiReprocFlgs,
                                             const QString &fileLaiFit,
                                             const QString &fileLaiFitFlgs,
...
bool IsMonoDateProcessingNeeded() { return false; }

bool IsNDaysReprocessingNeeded() { return false; }
