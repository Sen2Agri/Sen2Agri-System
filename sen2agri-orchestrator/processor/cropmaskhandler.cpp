                                             ...
                                             const JobSubmittedEvent &event)
{
    ...
    ctx.SubmitTasks(event.jobId,
                    { bandsExtractor, clipPolys, clipRaster, sampleSelection, randomSelection,
                      temporalResampling, featureExtraction, temporalFeatures, statisticFeatures,
                      concatenateImages, computeImagesStatistics, trainImagesClassifier,
    ...
    QStringList trainImagesClassifierArgs = {
        "-io.il",      features,   "-io.vd",         randomTrainingPolys,
        "-io.imstat",  statistics, "-rand",          "42",
        "-sample.bm",  "0",        "-io.confmatout", confusionMatrix,
        "-io.out",     model,      "-sample.mt",     "-1",
    ...
    NewStepList steps = {
        ...
        sampleSelection.CreateStep("SampleSelection",
                                   { "SampleSelection", "-ref", refPolysClipped, "-ratio",
                                     ...
                                     "-nofilter", "true" }),
        randomSelection.CreateStep("RandomSelection",
                                   { "RandomSelection", "-ref", trainingPolys, "-nbtrsample",
                                     trainingSamplesNumber, "-seed", "0", "-trp",
                                     randomTrainingPolys, "-tsp", randomTestingPolys }),
        temporalResampling.CreateStep("TemporalResampling",
                                      { "TemporalResampling", "-tocr", tocr, "-mask", mask, "-ind",
                                        ...
                                        dateEnd, "-rtocr", rtocr, "-outdays", days }),
        ...
        statisticFeatures.CreateStep("StatisticFeatures", { "StatisticFeatures", "-ndwi", ndwi,
                                                            "-brightness", brightness, "-sf", sf }),
        ...
        computeConfusionMatrix.CreateStep("ComputeConfusionMatrix",
                                          { "-in", cropMask, "-out", confusionMatrixValidation,
                                            "-ref", "vector", "-ref.vector.in", validationPolys,
                                            "-ref.vector.field", fieldName })
    };
