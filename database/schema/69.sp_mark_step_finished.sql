...
BEGIN
	...
	WHERE id = _task_id
	AND status_id != 5 -- Prevent resetting the status on serialization error retries.
	AND NOT EXISTS (SELECT * FROM step WHERE task_id = _task_id AND status_id != 5); -- Check that all the steps have been finished.
	
	IF EXISTS (SELECT * FROM task WHERE id = _task_id AND status_id = 5) 
	...
	FROM task WHERE id = _task_id AND status_id = 5);
